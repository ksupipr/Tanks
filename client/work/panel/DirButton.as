	...
	public class DirButton extends MovieClip{
      ...
      loader.addEventListener(IOErrorEvent.IO_ERROR, function(event:Event){
		...
		public function set_reg(_pr:int):void{
			...
			quantity=_pr;
			ch_num();
			_this["cd_time"]=_this["cd_wait"]=100*(int(_this["reg"][0])/int(_this["reg"][1]));
		...
		public function begin_cd():void{
			...
			if(this["gr_slots"].length>0){
				var _cd:int=int.MAX_VALUE;
				for(var t:int=0;t<this["gr_slots"].length;t++){
					var _slot:MovieClip=this["gr_slots"][t];
					if(_slot["cd_time"]!=0&&_cd>_slot["cd_time"]){
						_cd=_slot["cd_time"];
					}
				}
				for(var t:int=0;t<this["gr_slots"].length;t++){
					...
					if(!_slot["empty"]){
						...
						_slot["cd_wait"]=_cd;
						try{
							_slot.removeEventListener(Event.ENTER_FRAME, drawMyRect);
						}catch(er:Error){}
						_slot.addEventListener(Event.ENTER_FRAME, drawMyRect);
					}
				}
			}else if(!this["empty"]){
				steps=_i;
		...
		public function drawMyRect(event:Event){
			...
			var cd_num:Number=((step_time-_cl["steps"])/(_cl["cd_wait"]))*100;
			if(_cl["reg"].length>1){
				_cl["quantity"]=cd_num;
				_cl.ch_num();
			}
			if(cd_num<100){
				//var cd_num:Number=(_cl["cd_left"]/_cl["cd_wait"])*100;
				if(cd_num>0){
					_cl["clip"].graphics.clear();
					if(cd_num<12.5){
