...
package org.igniterealtime.xiff.vcard
{
	...
	public class VCard extends EventDispatcher
	{
		 ...
		 */
		public static function delVCard( jid:UnescapedJID ){
			delete cache[ jid.resource ];
		}
		
		public static function testVCard( jid:UnescapedJID ){
			var cachedCard:VCard = cache[ jid.resource ] as VCard;
			if ( cachedCard ){
				if(!cachedCard._loaded){
					delVCard(jid);
				}
			}
		}
		...
		public static function getVCard( connection:XMPPConnection, jid:UnescapedJID ):VCard
		{
			...
			/*var cachedCard:VCard = cache[ jid.resource ] as VCard;
			if ( cachedCard ){
				//trace(cachedCard+"   "+cachedCard.role+"   "+cachedCard._loaded);
				return cachedCard;
			}*/
