...
package org.igniterealtime.xiff.conference
{
	...
	public class Room extends ArrayCollection
	{
		...
		public function join( createReserved:Boolean = false, joinPresenceExtensions:Array = null ):Boolean
		{
			...
			//trace(muc+"   "+createReserved+"   "+joinPresenceExtensions+"   "+joinWithExplicitMUCExtension);
			if ( password != null )
			{
		...
		public function sendMessage( body:String = null, htmlBody:String = null ):void
		{
			if ( isActive )
			{
				...
				_connection.send( message );
				//trace("message   "+message+"   "+body+"   "+htmlBody);
			}
		}
		...
		private function updateRoomRoster( aPresence:Presence ):void
		{
			...
			var roomEvent:RoomEvent;
			//trace(isThisUser( aPresence.from.unescaped )+"   "+userJID.toString().toLowerCase()+"   "+aPresence.from.unescaped.toString().toLowerCase());
		...
		public function set isActive(_b:Boolean):void
		{
			_active=_b;
		}
		...
		public function set nickname( value:String ):void
		{
			...
			if ( isActive )
			{
				...
				var presence:Presence = new Presence( new EscapedJID( value ) );
