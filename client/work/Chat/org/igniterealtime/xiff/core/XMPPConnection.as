	...
	public class XMPPConnection extends EventDispatcher
	{
		...
		protected function flushPresenceQueue(event:TimerEvent):void
		{
			if(presenceQueue.length > 0) {
				//presenceQueue[0].from=new EscapedJID("reid@conference.xlab.su/vk_id91521807_ⓋⓀⓄⓃⓉⒶⓀⓉⒺ");
				var presenceEvent:PresenceEvent = new PresenceEvent();
				/*trace(presenceQueue[0].from+"   "+presenceQueue[0].to+"   "+presenceQueue[0].type);
				trace(presenceQueue[0].priority+"   "+presenceQueue[0].show+"   "+presenceQueue[0].status);
				trace(presenceQueue[0].id+"   "+presenceQueue[0]._exts);*/
