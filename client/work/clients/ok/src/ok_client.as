package {
	...
	public class ok_client extends MovieClip{
		...
		public function val_conv(_type:int,_n:Number):Number{
			if(_type==0){
				return ((_n/sn_p)*vl_p)/100;
			}
			return ((_n/vl_p)*sn_p)/100;
		}
		...
		public function ok_client(){
			...
			_self=this;
			xFormat.initFormat(_self,ok_client);
			console=new Console();
		...
		public function getAppParams(){
			MouseWheelTrap.setup(stage,ok_client);
			...
			us_data.ref=(params['refplace']);
			us_data.poster=(params['referer']);
			us_data.post="";
			us_data.is_app=(params['authorized']);
			
			par_ar[0]=(params['oid']);
			par_ar[1]=(params['group_id']);
			par_ar[2]=(params['logged_user_id']);
			par_ar[3]=(params['referer_type']);
			par_ar[4]=(params['auth_key']);
			par_ar[5]=(params['app_id']);
			par_ar[6]=(params['api_url']);
			par_ar[7]=(params['is_app_user']);
			par_ar[8]=(params['secretKey']);
			...
			ForticomAPI.addEventListener(ForticomAPI.CONNECTED, ForticomReady);
			ForticomAPI.connection = flashVars["apiconnection"];
		}
		
		public function ForticomReady(event:Event){
			Odnoklassniki.initialize(this, par_ar[8]);
			Odnoklassniki.addEventListener(ApiServerEvent.CONNECTED, okReadyHandler);
			...
			Odnoklassniki.addEventListener(ApiCallbackEvent.CALL_BACK, this.handleApiEvent);
		}
		
		private function okReadyHandler ( event : Event ) : void {
			console.status("OK API ready\n",1);
		}
		
		public function onErrorConnection(event:ApiServerEvent){
			errTest("[font=\"_sans\" size=\"17\" color=\"#ff0000\" textAlign=\"center\"][(]Odnoklassniki Api Connection Error[)]"+event.toString(), 100);
		}
		
		private var publish:Object=new Object();
		private var ok_reg:Object=new Object();
		
		protected function handleApiEvent(event : ApiCallbackEvent) : void{
			trace("ApiCallback function\n"+event.method+"\n"+event.result+"\n"+event.data);
			if(event.method=="showPayment"){
				if(event.result=="ok"){
					paym=1;
					try{
						cl_class.shop["exit"].buy_mem("buy");
						cl_class.shop["buy_cred_win"].visible=false;
					}catch(er:Error){
						cl_class.shop["exit"].sendRequest(["query","action"],[["id"],["id","prof","skills","things"]],[["2"],["1","0","1","1"]]);
					}
				}else{
					paym=0;
				...
				if(event.result=="ok"){
					ok_reg.resig = event.data;
					Odnoklassniki.callRestApi("stream.publish", onStreamPublish, ok_reg);
				}
			}/*else if(event.method=="showInvite"){
				trace("showInvite");
			}*/
		}
		
		public function onStreamPublish(d:Object):void{
			trace("onStreamPublish");
			for(var str:String in d){
				trace(str+"="+d[str]);
			}
		}
		...
		private var sms_prices:Array=[1,3,5];
		private var paym:int=0;
		
		public function showMoneyWin(_num:Number){
			var _ar:Array=[_num,String(Number(val_conv(1,_num)*100)+" "+cl_class.shop["exit"].m_name(Number(val_conv(1,_num)*100),3)),_num];
			ForticomAPI.showPayment(_ar[1], 'Это универсальная игровая валюта.', ""+_ar[0], _ar[2]/100, null, null, 'ok', 'true');
		}
		
		public function callFriends(){
			ForticomAPI.showInvite("Пошли играть в танчики!");
		}
		...
		private function completeHandler(event:Event):void {
			...
			_sendRequest("users.getInfo",{"lstnr_fnc":getUserInfoCallback,"params":{uids:(par_ar[2]),
				fields:"uid,first_name,last_name,name,gender,birthday,age,locale,pic_1,pic_2,pic_3,pic_4,url_profile"}});
		}
		
		private function getUserInfoCallback ( users : Object ) : void {
			/*trace("getUserInfoCallback   "+users);
			for(var str:String in users){
			trace(str+"="+users[str]);
			}*/
			if(int(users["error_code"])!=0){
				errTest(users["error_msg"], 150);
				us_data.was_burn="";
				us_data.nick="";
				us_data.sex=0;
				us_data.city="";
			...
			var _data:Object=users[0];
			/*for(var str:String in _data){
			trace(str+"="+_data[str]);
			}*/
			//if(par_ar[2]==Number(users[0].uid)){
			us_data.was_burn=_data.birthday;
			us_data.nick=_data.name;
			us_data.sex=_data.gender;
			us_data.city=_data.city;
			us_data.country=_data.country;
			us_data.rate="";
			us_data.m_phone="";
			us_data.h_phone="";
			us_data.friends="";
			...
			var _s_n:String=(_data["first_name"]+" "+_data["last_name"]);
			var re:RegExp =  /\"/gi;
			_s_n=_s_n.replace(re, "'");
			par_ar[14]=_s_n;
			if(!_data["url_profile"]){
				_link="http://www.odnoklassniki.ru/profile/"+_data["uid"];
			}else{
				_link=_data["url_profile"];
			}
			//trace(par_ar[14]+"   "+_link);
			try{
				auto_tx.text="Дождитесь завершения авторизации...";
			}catch(er:Error){}
			//}
		...
		private function allreadyLoaded():void {
			...
			try{
				kit.swf["utils"].init({self:this,self_class:ok_client});
			}catch(er:Error){
		...
		public function _sendRequest(method:String,send_obj:Object=null,_num:int=-1):void{
			...
			console.status("<b><font color=\"#000000\" size=\"11\">OK API Request:</font></b> "+method+"\n");
			if(_num>=0){
				requests[_num][2]=1;
			}else{
			...
			try{
				var lstnr_fnc:Function=send_obj.lstnr_fnc;
				var params:Object=send_obj.params;
				if(params!=null){
					Odnoklassniki.callRestApi(method, lstnr_fnc, params);
				}else{
					Odnoklassniki.callRestApi(method, lstnr_fnc);
				}
				if(re_req.running){
					re_req.stop();
				}
				...
				console.status("<b><font color=\"#ff0000\" size=\"11\">OK API Send Error:</font></b> "+method+": "+error+"\n");
		...
		public function showWall(){
			...
			var _ar:Array=upl_pict.split('/').splice(3);
			publish._scr=_ar.join('/');
			var media:Array = [ { "src":publish._scr, "type":"image" } ];
			var _publishAttachment:Object = { "caption":publish._title, "media":media };
			
			publish._text=_text;
			publish._title=_title;
			publish._mess=publish._title+"\n"+publish._text;
			publish._attachment=json.JSON.encode(_publishAttachment);
			
			ok_reg = {method : "stream.publish", message : publish._mess};
			ok_reg.uid = par_ar[2];
			ok_reg.attachment = publish._attachment;
			ok_reg = Odnoklassniki.getSignature(ok_reg);
			Odnoklassniki.showConfirmation("stream.publish", publish._mess, ok_reg.sig);
		}
