package {
	...
	public class ml_client extends MovieClip{
		...
		public function ml_client(){
			...
			_self=this;
			xFormat.initFormat(_self,ml_client);
			console=new Console();
		...
		public function getAppParams(){
			MouseWheelTrap.setup(stage,ml_client);
			...
			if(!tested){
				...
				us_data.ref=(params['referer_type']);
				us_data.poster=(params['referer_id']);
				us_data.post="";
				us_data.is_app=(params['app_installed']);
			...
			if(par_ar[5]==625881){
				par_ar[8]=("8de6d757d0f183275a3353dd65e7eb09");
				par_ar[9]="";    //norm
			...
			initTm.addEventListener(TimerEvent.TIMER_COMPLETE, reInit);
			initTm.start();
			...
			MailruCall.addEventListener ( MailruCallEvent.PAYMENT_DIALOG_STATUS, showPaymentDialogCallback );
			MailruCall.addEventListener ( MailruCallEvent.INCOMING_PAYMENT, incomingPaymentCallback );
			
			MailruCall.init("tanks2",par_ar[8]);
		}
		
		private static const CUSTOM_JS_EVENT : String = 'customJSEvent';
		public var initTm:Timer=new Timer(3000);
		public var ini_st:int=0;
		
		public function reInit(event:TimerEvent){
			if(MailruCall.init("tanks2",par_ar[8])==false){
				try{
					initTm.reset();
				}catch(er:Error){}
				if(ini_st==0){
		...
		private function streamPublishHandler ( event : MailruCallEvent ) : void {
			if(event.data.status=="publishSuccess"){
				...
				console.status("<b><font color=\"#ff0000\" size=\"11\">WALL POST CANCEL:</font></b>\n");
			}else{
				console.status("<b><font color=\"#ff0000\" size=\"11\">WALL POST Error:</font></b> "+event+"\n");
			}
		}
		...
		private function showPaymentDialogCallback ( event : MailruCallEvent ) : void {
			//trace ( 'showPaymentDialogCallback(): ' + traceObject ( event.data ) );
			if(event.data.status=="closed"){
				//trace("closed Payment   "+paym);
				if(paym==0){
				...
				//trace("opened Payment   "+paym);
				paym=0;
			}
			//trace("paym   "+paym);
		}
		
		private function incomingPaymentCallback ( event : MailruCallEvent ) : void {
			//trace ( 'incomingPaymentCallback(): ' + traceObject ( event.data ) );
			if(event.data.status=="success"){
				//trace("incomingPayment   ok");
				paym=1;
		...
		public function showMoneyWin(_num:Number){
			...
			var _sms:Number=(_num*0.9)/(0.5*2700);
			//trace("showMoneyWin   "+_ar+"   "+_sms);
			var _sms_c:Number=0;
			for(var i:int=0;i<sms_prices.length;i++){
				if(_sms>sms_prices[i]){
					if(i+1<sms_prices.length){
						_sms_c=i+1;
					}else{
						_sms=0;
					}
		...
		private function getUserInfoCallback ( users : Object ) : void {
			...
			us_data.was_burn=users[0].birthday;
			us_data.nick=users[0].nick;
			us_data.sex=users[0].sex;
			try{
				us_data.city=users[0].location.city.name;
			}catch(er:Error){
				us_data.city="";
			}
			try{
				us_data.country=users[0].location.country.name;
			}catch(er:Error){
				us_data.country="";
			}
			us_data.rate=users[0].vip;
			us_data.email=users[0].email;
			us_data.m_phone="";
			us_data.h_phone="";
			us_data.friends="";
			
			var _s_n:String=(users[0].first_name+" "+users[0].last_name);
			var re:RegExp =  /\"/gi;
		...
		private function allreadyLoaded():void {
			...
			try{
				kit.swf["utils"].init({self:this,self_class:ml_client});
			}catch(er:Error){
		...
		public function _sendRequest(method:String,send_obj:Object=null,_num:int=-1):void{
			...
			console.status("<b><font color=\"#000000\" size=\"11\">MAIL API Request:</font></b> "+method+"\n");
			if(_num>=0){
				requests[_num][2]=1;
			}else{
			...
			try{
				...
				if(params!=null){
					MailruCall.exec ( method, lstnr_fnc, params);
				}else{
					MailruCall.exec ( method, lstnr_fnc);
				}
				if(re_req.running){
					re_req.stop();
				}
				...
				console.status("<b><font color=\"#ff0000\" size=\"11\">MAIL API Send Error:</font></b> "+method+": "+error+"\n");
